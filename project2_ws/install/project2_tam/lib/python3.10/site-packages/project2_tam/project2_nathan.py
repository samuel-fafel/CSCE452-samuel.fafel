...
class DrawTAM(Node):
	def __init__(self, node_name):
		...
		#Find number of line segments for each turtle
		#self.workload = len(self.segments) // self.num_turtles
		...
		for turtle_name in self.turtle_names:
			...
			#self.id += 1
			#self.turtle_obj = tam.Turtle(turtle_name, self.id)
			#self.turtle_objs.append(turtle_obj)
		...
		#for turtle_obj in self.turtle_objs:
			#Distribute line segments
			#self.distribute_segments(self.segments, self.num_turtles, turtle_obj, self.workload)
