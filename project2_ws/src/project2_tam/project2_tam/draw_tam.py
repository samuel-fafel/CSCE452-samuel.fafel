...
class TamLogoDrawer(Node):
    ...
    def clear_and_set_background(self):
        clear = self.create_client(Empty, '/clear')
        ...
        # Set background color to Aggie maroon (you may need to customize the color)
        self.set_background_color(80, 0, 0)

    def set_background_color(self, r, g, b):
        self.get_logger().info(f'Setting background color to RGB: {r}, {g}, {b}')
        self.set_parameters([r, g, b], '/background_b', '/background_r', '/background_g')

    def set_parameters(self, *values, *param_names):
        for value, param_name in zip(values, param_names):
            self.get_logger().info(f'Setting {param_name} to {value}')
